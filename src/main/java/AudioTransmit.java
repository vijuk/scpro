...
public class AudioTransmit {
	...
	public AudioTransmit(final MediaLocator locator, final String ipAddress,
			final String port) {
	...
	private String createProcessor() {
		...
		for (final TrackControl track : tracks) {
			final Format format = track.getFormat();
			if (track.isEnabled() && format instanceof VideoFormat
					&& !programmed) {

				// Found a video track. Try to program it to output JPEG/RTP
				// Make sure the sizes are multiple of 8's.
				final Dimension size = ((VideoFormat) format).getSize();
				final float frameRate = ((VideoFormat) format).getFrameRate();
				final int w = (size.width % 8 == 0 ? size.width
						: (size.width / 8) * 8);
				final int h = (size.height % 8 == 0 ? size.height
						: (size.height / 8) * 8);
				final VideoFormat jpegFormat = new VideoFormat(
						VideoFormat.JPEG_RTP, new Dimension(w, h),
						Format.NOT_SPECIFIED, Format.byteArray, frameRate);
				track.setFormat(jpegFormat);
		...
		final ContentDescriptor cd = new ContentDescriptor(
	...
	private String createTransmitter() {
		...
		final String rtpURL = "rtp://" + ipAddress + ":" + port + "/video";
		final MediaLocator outputLocator = new MediaLocator(rtpURL);
	...
	public static void main(final String[] args) {
		...
		final AudioTransmit vt = new AudioTransmit(new MediaLocator(args[0]),
				args[1], args[2]);
		// Start the transmission
		final String result = vt.start();
